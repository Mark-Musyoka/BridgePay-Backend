...
async def test_refresh_token_reuse_is_detected_and_revokes_session(client):
    ...
    first_refresh = await client.post("/auth/refresh", json={"refresh_token": original_refresh})
    assert first_refresh.status_code == 200
    rotated_refresh = first_refresh.json()["refresh_token"]

    # Replaying the already-used original token should be rejected...
    replay_response = await client.post("/auth/refresh", json={"refresh_token": original_refresh})
    ...
    followup_response = await client.post("/auth/refresh", json={"refresh_token": rotated_refresh})
